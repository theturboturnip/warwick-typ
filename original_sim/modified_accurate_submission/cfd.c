...
int main(int argc, char *argv[])
{
    ...
    int itermax = 100000;        /* Maximum number of iterations in SOR */
    float eps = 0.01;        /* Stopping error threshold for SOR */
    ...
    float **u, **v,
        ...
        **f, **g;
    int **fluidmask, **surroundmask_red, **surroundmask_black;
    char  **flag;
    ...
    // Always allocate one more than needed??
    const int half_mat_height = ((jmax+2)/2)+1;
    fluidmask = allocIntMatrix(imax+2, jmax+2);
    surroundmask_red = allocIntMatrix(imax+2, half_mat_height);
    surroundmask_black = allocIntMatrix(imax+2, half_mat_height);
    ...
    if (!u || !v || !f || !g
        ...
        || !rhs || !rhs_red || !rhs_black
        || !flag
        || !fluidmask || !surroundmask_red || !surroundmask_black) {
    ...
    /* Create fluidmask */
    calculateFluidmask(fluidmask, flag, imax, jmax);
    splitFluidmaskToSurroundedMask(fluidmask, surroundmask_red, surroundmask_black, imax, jmax);
